...
use defmt::Format;
// use rustBoot_hal::nrf::nrf9160::GLOBAL_UART;
...
impl<Part: ValidPart> PartDescriptor<Part> {
    ...
    pub fn open_partition(part: Part, updater: impl FlashApi) -> Result<ImageType<'static>> {
        match part.part_id() {
            PartId::PartBoot => {
                // defmt::println!("Boot parition matched");
                let size;
                unsafe {
                    ...
                    if (magic != RUSTBOOT_MAGIC) || (size > PARTITION_SIZE - IMAGE_HEADER_SIZE) {
                        // defmt::println!("Invalid image. Bootpartion addr {:?} partition size {:?},  Image header size {:?}", 
                        // BOOT_PARTITION_ADDRESS, PARTITION_SIZE, IMAGE_HEADER_SIZE);
                        return Err(RustbootError::InvalidImage);
                    }
                }
                // defmt::println!("Boot parition matched 1");
                ...
                // defmt::println!("Boot parition matched 2");
                match part_desc.get_part_status(updater)? {
                    States::New(state) => {
                        // defmt::println!("Boot in New state");
                        Ok(ImageType::BootInNewState(RustbootImage {
                        part_desc: unsafe {
                    ...
                    States::Testing(state) => {
                        // defmt::println!("Boot in New state");
                        Ok(ImageType::BootInTestingState(RustbootImage {
                        part_desc: unsafe {
                            BOOT.get_or_init(|| part_desc);
                            // defmt::println!("Boot in Testing state");
                            &mut BOOT
                        },
                        state: Some(state),
                    }))
                },
                    States::Success(state) => {// defmt::println!("Boot in New state");
                    Ok(ImageType::BootInSuccessState(RustbootImage {
                        part_desc: unsafe {
                            BOOT.get_or_init(|| part_desc);
                            // defmt::println!("Boot in Success state");
                            &mut BOOT
                        },
                        state: Some(state),
                    }))},
                    _ => {
                        // defmt::println!("Nothing matched");
                        todo!()
                    }//todo!(),
                }
            }
            PartId::PartUpdate => {
                // defmt::println!("Update parition matched");
                let size;
                unsafe {
...
impl<Part: ValidPart + Swappable> PartDescriptor<Part> {
    pub fn get_part_status(&self, updater: impl FlashApi) -> Result<States> {
        // defmt::println!("getting partition status");
        let magic_trailer = unsafe { *self.get_partition_trailer_magic()? };
        // defmt::println!("getting partition status 1");
        if magic_trailer != RUSTBOOT_MAGIC_TRAIL as u32 {
            // defmt::println!("trailer magic mis matched");
        ...
        // defmt::println!("getting partition status 2");
        
        // defmt::println!("state is {:?}", my_state);
        ...
        // let my_state = state.as_ref();
        // // defmt::println!("state is {:?}", my_state.unwrap());
        state
    }
    ...
    fn get_trailer_at_offset(&self, offset: usize) -> Result<*const u8> {
        match self.trailer {
            Some(trailer_addr) => {
                Ok((trailer_addr as usize - (4 + offset)) as *const u8)
            }
            None => {
                return Err(RustbootError::FieldNotSet);
            }
        }
    }
