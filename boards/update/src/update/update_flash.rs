...
use rustBoot_hal::{FlashInterface, boot_from };//nrf::nrf9160::GLOBAL_UART};
...
// static GLOBAL_UART: spin::Mutex<Option<nrf9160_hal::uarte::Uarte<nrf9160_hal::pac::UARTE0_NS>>> =
// 	spin::Mutex::new(None);
    ...
    Interface: FlashInterface,
{
    ...
    fn flash_trailer_write<Part: ValidPart + Swappable>(
        ...
        // defmt::println!("Writting trailer magic {:?}, of len {:?} at address {:?}", data.clone(), len.clone(), addr.clone());
        self.iface.hal_flash_write(addr, data, len)
    }
    ...
    fn rustboot_start(self) -> ! {
        ...
        match boot {
            ImageType::BootInNewState(img) => {
                ...
                .0;
                hal_preboot();
                // defmt::println!("base_img_addr : {:?}", base_img_addr.clone());
    ...
    fn update_trigger(self) -> Result<()> {
        ...
        match updt {
            ...
            _ => {
                ...
                return Err(RustbootError::Unreachable)
            }
        }
        Self::flash_lock();
        Ok(())
