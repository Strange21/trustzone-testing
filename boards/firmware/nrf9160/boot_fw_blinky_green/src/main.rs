...
pub fn nvic_systemreset() -> ! {
    let core_peripherals = cortex_m::Peripherals::take().unwrap();//nrf9160_hal::pac::Peripherals::take().unwrap();
...
fn main()-> ! {   
    ...
    // let mut serial = Uarte::new(uarte0, pins, uarte::Parity::EXCLUDED, uarte::Baudrate::BAUD115200);
    // unsafe {
    //     *GLOBAL_UART.lock() = Some(serial);
    // }
    ...
    //         let mut buf = [0xA5; 12];
    //         buf.copy_from_slice("HelloWorld \n".as_bytes());
    ...
    // buf.copy_from_slice("NewwwWorld".as_bytes());
